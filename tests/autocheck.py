...
def zip_integrity():
    zpath = ROOT / "neuralscreen-v1.5.7-full.zip"
    if not zpath.is_file():
        return False, "no neuralscreen-v1.5.7-full.zip"
    required = [
    ...
    with zipfile.ZipFile(zpath) as z:
        ...
        if "NeuralScreen 1.5.7" not in vt:
            return False, "VERSION.txt version does not match v1.5.7"
